...
When changes are made on a non-main branch and a pull request is created toward `main`, the build-and-test job runs so the changes can be checked. However, the deployment job does not run because it is only allowed when the workflow is triggered by a push to the `main` branch.